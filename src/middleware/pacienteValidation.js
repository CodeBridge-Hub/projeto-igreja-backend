...
const CadastroValidacao = [
    ...
    body('condicao_saude_outro')
        .custom((value, { req }) => {
            // Assume que 'condicao_saude' é um array de strings (como ['Diabetes', 'Outro'])
    ...
    body('situacao_empregaticia')
        .notEmpty().withMessage('A situação empregatícia é obrigatória.') // CORREÇÃO: Colocamos o withMessage correto aqui
...
module.exports = {
    CadastroValidacao, // Para POST (Tela 1)
    EnderecoValidacao, // Para PATCH (Tela 2)
    OcupacaoValidacao // Para PATCH (Tela 3)
}